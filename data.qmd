...
questionnaires_raw    <- read_excel("data/raw/questionnaires.xlsx")
```
...
From the above lists of missing values (here we show it again and take a closer look at the missing variables for questionnaires data), we can tell that few children didn't reply to several questions in the questionnaire while after taking a closer look at the original data, we found out the 3 students didn't reply to any questions in the questionnaire at 3 different dates. It's possible that they just didn't took the questionnaire and thus we remove the whole row as it doesn't given any useful information.
...
clean_questionnaires <- questionnaires_raw |>
  ...
  mutate(
    ...
    week = session # Normalize participant ID
  )|>
  select(participant_id, date, week, adaquate, predilec, enjoy, scale_tot, 
         ada_m, pred_m, enjoy_m, scale_m) |>
  drop_na()
