...
questionnaires_raw      <- read_excel("data/raw/questionnaires.xlsx", sheet = 3)
recall_raw    <- read_excel("data/raw/24_hour_recalldata.xlsx")[-1,]
recall_raw <- recall_raw |>
  select(`Volunteer Number:`, `Week:`, `(10.1)  How much time did you spend in Sports/Active Games?`)
```
...
missing_questionnaires    <- missing_by_column(questionnaires_raw)
...
questionnaires_miss_long <- questionnaires_raw |>
  mutate(row_id = row_number()) |>
  pivot_longer(
    cols = c(Date, paste0("Q", 1:19)),
    names_to = "variable",
    values_to = "value",
    values_transform = list(value = as.character)
    ) |>
...
# Aggregate to identify all missingness patterns
questionnaires_miss_agg <- questionnaires_miss_long |>
  select(row_id, variable, missing) |>
  pivot_wider(names_from = variable, values_from = missing, values_fn = max) |>
  group_by(across(-row_id)) |>
  summarise(n_rows = n(), .groups = "drop") |>
...
# Create a lookup vector for the labels so ggplot knows 
# that pattern_id 1 = "48 rows...", pattern_id 2 = "2 rows...", etc.
y_label_map <- setNames(questionnaires_miss_agg$y_label_text, questionnaires_miss_agg$pattern_id) # <--- FIX

# Convert back to long for plotting
questionnaires_miss_agg_long <- questionnaires_miss_agg |>
  pivot_longer(cols = -c(n_rows, percent, percent_label, pattern_id, y_label_text), # <--- FIX: include new cols
               names_to = "variable",
               values_to = "missing")

# Get missing count per variable for plot ordering (Same as before)
var_missing_order <- questionnaires_miss_long |>
  group_by(variable) |>
  summarise(var_missing = sum(missing), .groups = "drop")

# Join order back
questionnaires_miss_agg_long <- questionnaires_miss_agg_long |>
  left_join(var_missing_order, by = "variable")

# Plot
ggplot(questionnaires_miss_agg_long,
       aes(
         x = reorder(variable, var_missing),
         # <--- FIX: Map Y to the unique ID, but order by count (n_rows)
         y = reorder(as.factor(pattern_id), n_rows), 
...
From the above lists of missing values (here we show it again and take a closer look at the missing variables for questionnaires data), we can tell that few children didn't reply to several questions in the questionnaire while after taking a closer look at the original data, we found out that only 2 students didn't reply to more than half of the questions in the questionnaire at 2 different dates. Thus we remove the whole row as it doesn't give any useful information.
...
```{r, results='hide'}
# Clean and process the data
clean_questionnaires <- questionnaires_raw |>
  janitor::clean_names() |>
  mutate(
    participant_id = as.numeric(gsub("406-", "", id)),
    week = session
  )|>
  select(!paste0("q", 1:19)) |>
  drop_na()

skimr::skim(clean_questionnaires)

# Save the cleaned data
write_csv(clean_questionnaires, "data/clean/clean_questionnaires.csv")
```
```{r, results='hide'}
clean_recall <- recall_raw |>
  janitor::clean_names()|>
  mutate(
    participant_id = volunteer_number,
    week = week,
    time = x10_1_how_much_time_did_you_spend_in_sports_active_games
  )|>
  drop_na()

skimr::skim(clean_recall)

write_csv(clean_recall, "data/clean/clean_recall.csv")
```
