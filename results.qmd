...
compq <- compq |>
  rename(
    external_regulation     = external_t,
    introjected_regulation  = introjec_t,
    identified_regulation   = identi_t,
    intrinsic_regulation    = intrinsic_t
  ) |>
  select(-amotiv_t)

mot_vars <- c("intrinsic_regulation", "external_regulation", 
              "introjected_regulation", "identified_regulation")

compq_long <- compq |>
  pivot_longer(
    cols = all_of(mot_vars),
    names_to = "motivation_type",
    values_to = "motivation_score"
  )

ggplot(compq_long, aes(x = motivation_score, y = mvpa)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ motivation_type, scales = "free_x") +
  labs(
    title = "MVPA vs Motivation Subscales",
    x = "Motivation Score",
    y = "MVPA (minutes/day)"
  ) +
  theme_minimal(base_size = 14)
...
Besides some data at the 0 points, which could be led by children not replying to the questionnaire. We can see a pattern that as the intrinsic regulation, introjected regulation and identified regulation increases, the average MVPA (or range) increases and the standard deviation also increases a bit. This results from the BREQ-2 shows that internal motivations to play active video games would lead to increasing potential of performing longer time moderate to vigorous physical activities
